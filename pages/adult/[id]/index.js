...
import adultzipData from '../../../public/adultzip.json'
import adultzipItemsData from '../../../public/moviesfull.json'
...
// Fetch data from adultzipItems.json
const fetchadultzipData = async () => {
  const response = await fetch('https://popcornfliks.vercel.app/moviesfull.json')
  return await response.json()
}

const getRandomLinks = (adultzipItems, count = 3) => {
  ...
  return [
    ...getRandomItems(adultzipItems, count)
...
const adultzipDetail = ({ adultzipItem }) => {
  ...
  // Determine the type of content (adultzipItem, tvshow, or adult)
  const isTVShow = adultzipItem.type === 'tvshow'
  const isAdult = adultzipItem.badgegroup === 'Adult'
  const [randomadultzipItems, setRandomadultzipItems] = useState([])
  const [linkTargets, setLinkTargets] = useState([])

  const enhancedParagraph = (text, adultzipItem) => {
    // Ensure adultzipItem and words are valid
    const words = Array.isArray(adultzipItem?.words) ? adultzipItem.words : []
    const videoadultzipItems = adultzipItem?.videoadultzipItems || ''
    const imdb = adultzipItem?.imdb || ''

    // Define link targets
    const linkTargets = [
      {
        text: words[0] || '', // Fallback to empty string if words[0] is undefined
        url: `https://www.imdb.com/title/${videoadultzipItems || imdb}/`
      }
    ]
  ...
  useEffect(() => {
    ...
    setLinkTargets(getRandomLinks(adultzipItemsData))
  ...
  const fetchData = async () => {
    try {
      ...
      const randomadultzipItemsData = getRandomItems(data, 5)
      setRandomadultzipItems(randomadultzipItemsData)
    } catch (error) {
  ...
  useEffect(() => {
    const detectMobileDevice = () => {
      ...
      const mobile = Boolean(
        userAgent.match(
          /Android|iPhone|iPad|iPod|Opera Mini|IEMobile|WPadultzipItems/i
        )
      )
  ...
  if (!adultzipItem) return <div>Loading...</div>

  const { videomovieitem, videomovies, image1 } = adultzipItem
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'adultzipItems',
        item: adultzipItem.baseurl
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: adultzipItem.name,
        item: adultzipItem.siteurl
      }
    ]
  })
  ...
  const rankMathSchema = JSON.stringify({
    ...
    '@graph': [
      ...
      {
        '@type': 'WebPage',
        '@id': `${adultzipItem.siteurl}#webpage`,
        url: adultzipItem.siteurl,
        name: `${adultzipItem.name} | Popcornflix`,
        datePublished: adultzipItem.datePublished,
        dateModified: adultzipItem.dateModified,
        isPartOf: {
          '@id': 'https://popcornfliks.vercel.app#website'
        },
        inLanguage: 'en-US'
      ...
      {
        '@type': 'Article',
        '@id': `${adultzipItem.siteurl}#article`,
        headline: ` ${adultzipItem.name} | Popcornflix`,
        datePublished: adultzipItem.datePublished,
        dateModified: adultzipItem.dateModified,
        articleSection: 'adultzipItems',
        author: {
          '@id': 'https://gravatar.com/drtrailer2022/'
        },
        publisher: {
        ...
        description: adultzipItem.synopsis,
        image: adultzipItem.image,
        name: ` ${adultzipItem.name} | Popcornflix`,
        isPartOf: {
          '@id': `${adultzipItem.siteurl}#webpage`
        },
      ...
      {
        '@type': 'BlogPosting',
        '@id': `${adultzipItem.siteurl}#blogPost`,
        headline: ` ${adultzipItem.name} | Popcornflix`,
        datePublished: adultzipItem.datePublished,
        dateModified: adultzipItem.dateModified,
        articleSection: 'adultzipItems',
        ...
        image: adultzipItem.image,
        name: ` ${adultzipItem.name} | Popcornflix`,
        '@id': `${adultzipItem.siteurl}#richSnippet`,
        isPartOf: {
          '@id': `${adultzipItem.siteurl}#webpage`
        },
        inLanguage: 'en-US',
  ...
  const newsArticleSchema = {
    '@context': 'https://schema.org',
    '@type': 'NewsArticle',
    '@id': `${adultzipItem.siteurl}#webpage`, // Add a comma here
    name: adultzipItem.title,
    url: adultzipItem.siteurl,
    description: adultzipItem.synopsis,
    image: adultzipItem.image,
    datePublished: adultzipItem.startDate,
    potentialAction: {
      '@type': 'WatchAction',
      target: {
        '@type': 'EntryPoint',
        name: adultzipItem.title,
        urlTemplate: adultzipItem.siteurl
      }
    },
    locationCreated: {
      '@type': 'Place',
      name: adultzipItem.country
    },
    author: {
      '@type': 'Person',
      name: 'DrTrailer',
  ...
  const ldJsonData = JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'adultzipItem',
    '@id': `${adultzipItem.siteurl}`,
    name: adultzipItem.title,
    url: adultzipItem.siteurl,
    description: adultzipItem.synopsis,
    image: adultzipItem.image,
    genre: adultzipItem.genre,
    datePublished: adultzipItem.datePublished,
    director: {
      '@type': 'Person',
      name: adultzipItem.directorname
    },
    actor: adultzipItem.starring.map(actor => ({
      '@type': 'Person',
      name: actor
    })),
    potentialAction: {
  ...
  const adultzipItemsSchema = JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'VideoObject',
    name: adultzipItem.title,
    description: adultzipItem.text,
    uploadDate: adultzipItem.datePublished,
    thumbnailUrl: adultzipItem.image1,
    duration: 'P34S', // Replace with the actual duration if it's different
    embedUrl: adultzipItem.videourl
  })
  ...
  return (
    <div>
      <Head>
        ...
        />
        <title> {adultzipItem.name} | Popcornflix</title>
        <link rel='canonical' href={adultzipItem && adultzipItem.siteurl} />
        ...
        <meta
          property='og:title'
          content={`${adultzipItem && adultzipItem.name} - Popcornflix`}
        />
        <meta
          property='og:description'
          content='Stream HD adultzipItems and TV series for free on Popcornflix. Online. Stream. Download. full-length adultzipItems and shows in HD quality without registration.'
        />
        <meta
          name='description'
          content={`${adultzipItem.title} available on Popcornflix. Enjoy free streaming of full-length adultzipItems and TV series online with no registration required.`}
        />
        <meta
          property='og:url'
          content={`${adultzipItem && adultzipItem.siteurl}`}
        />
        <meta
          name='keywords'
          content={`${adultzipItem && adultzipItem.keywords}`}
        />
        <meta property='og:site_name' content='Popcornflix' />
        <meta
          property='og:image:alt'
          content={`${adultzipItem && adultzipItem.name}`}
        />
        <meta
          property='og:image'
          fetchpriority='high'
          as='image'
          content={adultzipItem.image1}
        />
        <meta name='mobile-web-app-capable' content='yes' />
        <meta property='article:section' content='adultzipItems' />
        <meta name='author' content='admin' />
        <meta
          property='article:modified_time'
          content='2024-01-01T13:13:13+00:00'
        />
        <meta property='og:type' content='video.adultzipItem' />
        {/* Specific to the adultzipItem page */}
        <meta property='og:video' content={adultzipItem.videourl} />
        <meta property='og:video:type' content='video/mp4' />
        <meta property='og:video:width' content='1280' />
        <meta property='og:video:height' content='720' />
        <meta property='og:image' content={adultzipItem.image1} />
        ...
        <meta
          name='twitter:image'
          content={`${adultzipItem && adultzipItem.image1}`}
        />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              name: 'Popcornflix',
              url: `${adultzipItem && adultzipItem.siteurl}`,
              potentialAction: {
                '@type': 'SearchAction',
                target: `${adultzipItem.siteurl}/search?q={search_term_string}`,
                'query-input': 'required name=search_term_string'
              }
            })
          }}
        ...
        <script
          type='application/ld+json'
          dangerouslySetInnerHTML={{ __html: adultzipItemsSchema }}
        />
        <script
          type='application/ld+json'
      ...
      <SocialSharing />
      {isAdult && <AdultSkipAds adultzipItem={adultzipItem} />}
      ...
      >
        ...
        {isTVShow && (
          <>
            <h2 className='px-0 bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-2xl hover:text-blue-800 font-bold mt-2'>
              {adultzipItem.title} Online - Stream Your Favorite TV Series
            </h2>
            <p className='text-lg text-yellow-500 mt-4'>
              Explore the captivating world of{' '}
              <strong>{adultzipItem.title}</strong>, the TV series that has
              everyone talking. At
              <strong> Popcornflix</strong>, you can stream{' '}
              <strong>{adultzipItem.title}</strong> and immerse yourself in its
            ...
            <p className='text-lg text-yellow-500 mt-4'>
              Streaming <strong>{adultzipItem.title}</strong> on{' '}
              ...
              need to enjoy <strong>{adultzipItem.title}</strong>
        ...
        {isAdult && (
          <>
            <h2 className='px-0 bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-2xl hover:text-blue-800 font-bold mt-2'>
              {adultzipItem.title} Online - Stream Premium Adult Content
            </h2>
            <p className='text-lg text-yellow-500 mt-4'>
              Indulge in the finest selection of adult entertainment with{' '}
              <strong>{adultzipItem.title}</strong>. At{' '}
              ...
              <strong>{adultzipItem.title}</strong>. Our platform is designed
            ...
            <p className='text-lg text-yellow-500 mt-4'>
              ...
              <strong>{adultzipItem.title}</strong> today and discover why{' '}
        ...
        {/* adultzipItem Description (Default) */}
        {!isTVShow && !isAdult && (
          <>
            <h2 className='px-0 bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-2xl  font-bold mt-2'>
              {adultzipItem.title} Online and Experience Top-Tier Streaming
            </h2>
            <p className='text-lg text-yellow-500 mt-4'>
              Dive into the world of cinema with{' '}
              <strong>{adultzipItem.title}</strong>, available to stream right
              here. At <strong>Popcornflix</strong>, we bring you the best in
              entertainment, offering an extensive library of adultzipItems and
              TV shows, including the latest blockbusters like{' '}
              <strong>{adultzipItem.title}</strong>. Whether you're a fan of
            ...
            <p className='text-lg text-yellow-500 mt-4'>
              ...
              ensuring you can watch <strong>{adultzipItem.title}</strong> and
            ...
            <p className='text-lg text-yellow-500 mt-4'>
              ...
              adultzipItem night. Join the countless users who trust us for
              their streaming needs and start watching{' '}
              <strong>{adultzipItem.title}</strong> online today. At{' '}
        ...
        {/* </div> */}
        <a
          href='https://t.me/watchadultzipItemtvshow/'
          target='_blank'
        ...
        >
          <p style={{ display: 'inline-block' }}>
            ...
            adultzipItems & TV Series Join Telegram
        ...
        >
          <div className='flex-container'>
            <div className='category-container'>
              <Image
                src={adultzipItem.image}
                alt={adultzipItem.title}
                width={300}
                height={300}
                quality={90}
                loading='lazy'
              ...
              >
                ...
                >
                  ...
                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Director: {adultzipItem.directorname}
                  </h2>
                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Starring: {adultzipItem.starring}
                  </h2>
                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Origin Country: {adultzipItem.country}
                  </h2>
                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Language: {adultzipItem.language}
                  </h2>

                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Total Episodes: {adultzipItem.episode}
                  </h2>
                  <div className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Synopsis :-
                    {adultzipItem.text &&
                      adultzipItem.text.split('\n\n').map((paragraph, idx) => (
                        <p
                          key={idx}
                          style={{
                            marginBottom: '10px',
                          ...
                          dangerouslySetInnerHTML={{
                            __html: enhancedParagraph(paragraph, adultzipItem) // Pass adultzipItem here
                          }}
                        />
                      ))}
                  </div>
                  ...
                  <div className={`${HomeStyles.imageGrid} mt-5`}>
                    <img
                      ...
                      src={adultzipItem.directorimg}
                      alt={adultzipItem.directorname}
                      title={adultzipItem.directorname}
                      quality={90}
                      style={{
                    ...
                    <img
                      ...
                      src={adultzipItem.actor1img}
                      alt={adultzipItem.actor1}
                      title={adultzipItem.actor1}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                    ...
                    <img
                      className={`${HomeStyles.image} img-fluid lazyload `}
                      src={adultzipItem.actor2img}
                      alt={adultzipItem.actor2}
                      title={adultzipItem.actor2}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                    ...
                    <img
                      className={`${HomeStyles.image} img-fluid lazyload `}
                      src={adultzipItem.actor3img}
                      alt={adultzipItem.actor3}
                      title={adultzipItem.actor3}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                    ...
                    <img
                      className={`${HomeStyles.image} img-fluid lazyload `}
                      src={adultzipItem.actor4img}
                      alt={adultzipItem.actor4}
                      title={adultzipItem.actor4}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                    ...
                    <img
                      className={`${HomeStyles.image} img-fluid lazyload `}
                      src={adultzipItem.actor5img}
                      alt={adultzipItem.actor5}
                      title={adultzipItem.actor5}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                  ...
                  <h2 className='px-0 bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-4xl hover:text-blue-800 font-bold mt-2'>
                    Watch Official Trailer {adultzipItem.name}
                  </h2>
                      ...
                      <div className={styles.iframeWrapper}>
                        <div className={styles.iframeContainer}>
                          <iframe
                            ...
                            frameBorder='0'
                            src={`https://geo.dailymotion.com/player/xjrxe.html?video=${adultzipItem.traileritem}&mute=true&Autoquality=1080p`}
                            allowFullScreen
                    ...
                    <>
                      ...
                      <div className='flex flex-col items-center mt-4 gap-2'>
                        ...
                        <div className='flex flex-col items-center justify-center'>
                          ...
                          {adultzipItem.linkurl && (
                            <Link href={adultzipItem.linkurl}>
                              <div
                          ...
                          {adultzipItem.linkurl2 && (
                            <Link href={adultzipItem.linkurl2}>
                              <div
                                className={`px-4 py-2 border rounded mx-2 my-1 ${
                                  adultzipItem.linkurl2
                                    ? 'bg-red-500 text-white'
                  ...
                  <div className={styles.container}>
                    <h2
                      ...
                      style={{ fontFamily: 'Poppins, sans-serif' }}
                    >
                      Click to Download {adultzipItem.name}
                    </h2>

                    {/* <div className='flex flex-col items-center justify-center'></div>

                  {adultzipItem.mp3player && <MP3Player mp3Url={adultzipItem.mp3player} />} */}
            ...
            <div className='sidebar'>
              ...
              <div className='categorylatest-container'>
                <div className='cardlatest-container'>
                  {randomadultzipItems.map(adultzipItems => (
                    <div key={adultzipItems.id} className='cardlatest'>
                      <a href={adultzipItems.siteurl} id={adultzipItems.id}>
                        <div className='relative'>
                          <img
                            src={adultzipItems.image}
                            alt={adultzipItems.title}
...
export async function getStaticPaths () {
  const paths = adultzipData.map(item => ({
    params: { id: item.id }
  }))
...
export async function getStaticProps ({ params }) {
  const adultzipItem = adultzipData.find(item => item.id === params.id)
  return { props: { adultzipItem } }
}
export default adultzipDetail
