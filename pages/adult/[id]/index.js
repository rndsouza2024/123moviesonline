...
import adult from "../../../public/adult.json"; // Ensure the correct path for your JSON file
...
export default function MovieDetail({ movie }) {
  ...
  const videoSources = videomovies.map((item) => {
    ...
    return {
      ...
      urls: [
        `https://short.ink/${currentVideoId}?thumbnail=${movie.image1}`,
        isItemMovies
      ...
      {isTVShow && (
        <>
          ...
          <p className={styles.year} style={{ textShadow: "1px 1px 0px #000" }}>
            Explore the captivating world of <strong>{movie.title}</strong>, the
            TV series that has everyone talking. At
            <strong> Moviesfree™</strong>, you can stream{" "}
            <strong>{movie.title}</strong> and immerse yourself in its exciting
          ...
          <p className={styles.year} style={{ textShadow: "1px 1px 0px #000" }}>
            ...
            need to enjoy <strong>{movie.title}</strong> and other top TV
            series. Our library is frequently updated, so you can always find
            the latest episodes as soon as they air.
          </p>
          <p className={styles.year} style={{ textShadow: "1px 1px 0px #000" }}>
      ...
      {isAdult && (
        <>
          ...
          <p className={styles.year} style={{ textShadow: "1px 1px 0px #000" }}>
            ...
            <strong>{movie.title}</strong>. At <strong>Moviesfree™</strong>, we
          ...
          <p className={styles.year} style={{ textShadow: "1px 1px 0px #000" }}>
            ...
            than <strong>{movie.title}</strong> on <strong>Moviesfree™</strong>
            ...
            today and discover why <strong>Moviesfree™</strong> is the trusted
...
export async function getStaticPaths() {
  ...
  const adult = JSON.parse(fileContents);

  const paths = adult.map((movie) => ({
    params: { id: movie.id },
  }));
...
export async function getStaticProps({ params }) {
  ...
  const movie = adult.find((movie) => movie.id === params.id);
