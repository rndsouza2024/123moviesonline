...
const adultDetail = ({ adult }) => {
  ...
  const languagesSchema = JSON.stringify({
    ...
    alternateName: [
      'Ver Backrider Parte (2024) para adultos | 123Movies™',
      'Regarder Backrider Partie (2024) pour adultes | 123Movies™',
      'Backrider Teil (2024) für Erwachsene ansehen | 123Movies™',
      '观看 Backrider (2024) 成人内容 | 123Movies™',
      'Backrider パート (2024) 大人向け | 123Movies™',
      'Backrider 파트 (2024) 성인 전용 | 123Movies™',
      'Assistir Backrider Parte (2024) para adultos | 123Movies™',
      'Guarda Backrider Parte (2024) per adulti | 123Movies™',
      'Посмотреть Backrider Часть (2024) для взрослых | 123Movies™',
...
export async function getServerSideProps () {
  ...
  const selectedAdult = data.find(adult => adult.id === 'INDEX24')
  return {
    props: {
      adult: selectedAdult
    }
