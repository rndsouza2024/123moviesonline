...
import movieszipData from '../../../public/movieszip.json'
import movieszipItemsData from '../../../public/moviesfull.json'
...
const getRandomLinks = (movieszipItems, count = 3) => {
  ...
  return [
    ...getRandomItems(movieszipItems, count)
...
const movieszipDetail = ({ movieszipItem }) => {
  ...
  // Determine the type of content (movieszipItem, tvshow, or adult)
  const isTVShow = movieszipItem.type === 'tvshow'
  const isAdult = movieszipItem.badgegroup === 'Adult'
  const [randommovieszipItems, setRandommovieszipItems] = useState([])
  const [linkTargets, setLinkTargets] = useState([])

  const enhancedParagraph = (text, movieszipItem) => {
    // Ensure movieszipItem and words are valid
    const words = Array.isArray(movieszipItem?.words) ? movieszipItem.words : []
    const videomovieszipItems = movieszipItem?.videomovieszipItems || ''
    const imdb = movieszipItem?.imdb || ''

    // Define link targets
    const linkTargets = [
      {
        text: words[0] || '', // Fallback to empty string if words[0] is undefined
        url: `https://www.imdb.com/title/${videomovieszipItems || imdb}/`
      }
    ]
  ...
  useEffect(() => {
    ...
    setLinkTargets(getRandomLinks(movieszipItemsData))
  ...
  const fetchData = async () => {
    try {
      ...
      const randommovieszipItemsData = getRandomItems(data, 5)
      setRandommovieszipItems(randommovieszipItemsData)
    } catch (error) {
  ...
  useEffect(() => {
    const detectMobileDevice = () => {
      ...
      const mobile = Boolean(
        userAgent.match(
          /Android|iPhone|iPad|iPod|Opera Mini|IEMobile|WPmovieszipItems/i
        )
      )
  ...
  if (!movieszipItem) return <div>Loading...</div>

  const { videomovieitem, videomovies, image1 } = movieszipItem
  ...
  const breadcrumbSchema = JSON.stringify({
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'movieszipItems',
        item: movieszipItem.baseurl
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: movieszipItem.name,
        item: movieszipItem.siteurl
      }
    ]
  })
  ...
  const newsArticleSchema = {
    '@context': 'https://schema.org',
    '@type': 'NewsArticle',
    '@id': `${movieszipItem.siteurl}#webpage`, // Add a comma here
    name: movieszipItem.title,
    url: movieszipItem.siteurl,
    description: movieszipItem.synopsis,
    image: movieszipItem.image,
    datePublished: movieszipItem.startDate,
    potentialAction: {
      '@type': 'WatchAction',
      target: {
        '@type': 'EntryPoint',
        name: movieszipItem.title,
        urlTemplate: movieszipItem.siteurl
      }
    },
    locationCreated: {
      '@type': 'Place',
      name: movieszipItem.country
    },
    author: {
      '@type': 'Person',
      name: 'DrTrailer',
  ...
  const ldJsonData = JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'movieszipItem',
    '@id': `${movieszipItem.siteurl}`,
    name: movieszipItem.title,
    url: movieszipItem.siteurl,
    description: movieszipItem.synopsis,
    image: movieszipItem.image,
    genre: movieszipItem.genre,
    datePublished: movieszipItem.datePublished,
    director: {
      '@type': 'Person',
      name: movieszipItem.directorname
    },
    actor: movieszipItem.starring.map(actor => ({
      '@type': 'Person',
      name: actor
    })),
    potentialAction: {
  ...
  const movieszipItemsSchema = JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'VideoObject',
    name: movieszipItem.title,
    description: movieszipItem.text,
    uploadDate: movieszipItem.datePublished,
    thumbnailUrl: movieszipItem.image1,
    duration: 'P34S', // Replace with the actual duration if it's different
    embedUrl: movieszipItem.videourl
  })
  ...
  return (
    <div>
      <Head>
        ...
        <meta
          property='og:url'
          content={`${movieszipItem && movieszipItem.siteurl}`}
        />
        <meta
          name='keywords'
        ...
        <meta
          ...
          fetchpriority='high'
          as='image'
          content={movieszipItem.image1}
        />
        <meta name='mobile-web-app-capable' content='yes' />
        <meta property='article:section' content='movieszipItems' />
        <meta name='author' content='admin' />
        <meta
          property='article:modified_time'
          content='2024-01-01T13:13:13+00:00'
        />
        <meta property='og:type' content='video.movieszipItem' />
        {/* Specific to the movieszipItem page */}
        <meta property='og:video' content={movieszipItem.videourl} />
        <meta property='og:video:type' content='video/mp4' />
        <meta property='og:video:width' content='1280' />
        <meta property='og:video:height' content='720' />
        <meta property='og:image' content={movieszipItem.image1} />
        ...
        <meta
          name='twitter:image'
          content={`${movieszipItem && movieszipItem.image1}`}
        />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              potentialAction: {
                '@type': 'SearchAction',
                target: `${movieszipItem.siteurl}/search?q={search_term_string}`,
                'query-input': 'required name=search_term_string'
              }
            })
          }}
        ...
        <script
          type='application/ld+json'
          dangerouslySetInnerHTML={{ __html: movieszipItemsSchema }}
        />
        <script
          type='application/ld+json'
      ...
      >
        ...
        {isTVShow && (
          <>
            <h2 className='px-0 bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-2xl hover:text-blue-800 font-bold mt-2'>
              {movieszipItem.title} Online - Stream Your Favorite TV Series
            </h2>
        ...
        {isAdult && (
          <>
            <h2 className='px-0 bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-2xl hover:text-blue-800 font-bold mt-2'>
              {movieszipItem.title} Online - Stream Premium Adult Content
            </h2>
            <p className='text-lg text-yellow-500 mt-4'>
            ...
            <p className='text-lg text-yellow-500 mt-4'>
              ...
              <strong>{movieszipItem.title}</strong>
        ...
        {!isTVShow && !isAdult && (
          <>
            ...
            <p className='text-lg text-yellow-500 mt-4'>
              ...
              ensuring you can watch <strong>{movieszipItem.title}</strong> and
        ...
        {/* </div> */}
        <a
          href='https://t.me/watchmovieszipItemtvshow/'
          target='_blank'
        ...
        >
          <p style={{ display: 'inline-block' }}>
            ...
            movieszipItems & TV Series Join Telegram
        ...
        >
          <div className='flex-container'>
            <div className='category-container'>
              <Image
                src={movieszipItem.image}
                alt={movieszipItem.title}
                width={300}
                height={300}
                quality={90}
                loading='lazy'
              ...
              >
                ...
                >
                  ...
                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Director: {movieszipItem.directorname}
                  </h2>
                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Starring: {movieszipItem.starring}
                  </h2>
                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Origin Country: {movieszipItem.country}
                  </h2>
                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Language: {movieszipItem.language}
                  </h2>

                  <h2 className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Total Episodes: {movieszipItem.episode}
                  </h2>
                  <div className='bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-bg font-semibold mt-2'>
                    Synopsis :-
                    {movieszipItem.text &&
                      movieszipItem.text.split('\n\n').map((paragraph, idx) => (
                        <p
                          key={idx}
                          style={{
                            marginBottom: '10px',
                          ...
                          dangerouslySetInnerHTML={{
                            __html: enhancedParagraph(paragraph, movieszipItem) // Pass movieszipItem here
                          }}
                        />
                      ))}
                  </div>
                  ...
                  <div className={`${HomeStyles.imageGrid} mt-5`}>
                    <img
                      ...
                      src={movieszipItem.directorimg}
                      alt={movieszipItem.directorname}
                      title={movieszipItem.directorname}
                      quality={90}
                      style={{
                    ...
                    <img
                      ...
                      src={movieszipItem.actor1img}
                      alt={movieszipItem.actor1}
                      title={movieszipItem.actor1}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                    ...
                    <img
                      className={`${HomeStyles.image} img-fluid lazyload `}
                      src={movieszipItem.actor2img}
                      alt={movieszipItem.actor2}
                      title={movieszipItem.actor2}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                    ...
                    <img
                      className={`${HomeStyles.image} img-fluid lazyload `}
                      src={movieszipItem.actor3img}
                      alt={movieszipItem.actor3}
                      title={movieszipItem.actor3}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                    ...
                    <img
                      className={`${HomeStyles.image} img-fluid lazyload `}
                      src={movieszipItem.actor4img}
                      alt={movieszipItem.actor4}
                      title={movieszipItem.actor4}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                    ...
                    <img
                      className={`${HomeStyles.image} img-fluid lazyload `}
                      src={movieszipItem.actor5img}
                      alt={movieszipItem.actor5}
                      title={movieszipItem.actor5}
                      quality={90}
                      style={{
                        objectFit: 'cover', // Ensures the image covers the container
                        boxShadow: '0 0 10px 0 #000', // Shadow effect with black color
                  ...
                  <h2 className='px-0 bg-gradient-to-r from-amber-500 to-pink-500 bg-clip-text text-transparent text-4xl hover:text-blue-800 font-bold mt-2'>
                    Watch Official Trailer {movieszipItem.name}
                  </h2>
                      ...
                      <div className={styles.iframeWrapper}>
                        <div className={styles.iframeContainer}>
                          <iframe
                            ...
                            frameBorder='0'
                            src={`https://geo.dailymotion.com/player/xjrxe.html?video=${movieszipItem.traileritem}&mute=true&Autoquality=1080p`}
                            allowFullScreen
                    ...
                    <>
                      ...
                      <div className='flex flex-col items-center mt-4 gap-2'>
                        ...
                        <div className='flex flex-col items-center justify-center'>
                          ...
                          {movieszipItem.linkurl && (
                            <Link href={movieszipItem.linkurl}>
                              <div
                          ...
                          {movieszipItem.linkurl2 && (
                            <Link href={movieszipItem.linkurl2}>
                              <div
                                className={`px-4 py-2 border rounded mx-2 my-1 ${
                                  movieszipItem.linkurl2
                                    ? 'bg-red-500 text-white'
                  ...
                  <div className={styles.container}>
                    <h2
                      ...
                      style={{ fontFamily: 'Poppins, sans-serif' }}
                    >
                      Click to Download {movieszipItem.name}
                    </h2>

                    {/* <div className='flex flex-col items-center justify-center'></div>

                  {movieszipItem.mp3player && <MP3Player mp3Url={movieszipItem.mp3player} />} */}
            ...
            <div className='sidebar'>
              ...
              <div className='categorylatest-container'>
                <div className='cardlatest-container'>
                  {randommovieszipItems.map(movieszipItems => (
                    <div key={movieszipItems.id} className='cardlatest'>
                      <a href={movieszipItems.siteurl} id={movieszipItems.id}>
                        <div className='relative'>
                          <img
                            src={movieszipItems.image}
                            alt={movieszipItems.title}
...
export async function getStaticPaths () {
  const paths = movieszipData.map(item => ({
    params: { id: item.id }
  }))
...
export async function getStaticProps ({ params }) {
  const movieszipItem = movieszipData.find(item => item.id === params.id)
  return { props: { movieszipItem } }
}
export default movieszipDetail
