...
const moviesDetail = ({ movie }) => {
  ...
  <title>Watch Movie A Quiet Place Day One (2024) | 123 Movies™</title>
...
export async function getServerSideProps () {
  const res = await fetch('https://123movies-free.vercel.app/moviesp3.json')
  const data = await res.json()
  const selectedMovie = data.find(
    movie => movie.id === 'watch-a-quiet-place-day-one-2024'
  )
  return {
    props: {
      movie: selectedMovie
