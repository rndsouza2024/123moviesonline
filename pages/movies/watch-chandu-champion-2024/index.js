...
const moviesDetail = ({ movie }) => {
  ...
  <title>Watch Movie Chandu Champion (2024) | 123 Movies™</title>
...
export async function getServerSideProps () {
  const res = await fetch('https://123movies-free.vercel.app/moviesp4.json')
  const data = await res.json()
  const selectedMovie = data.find(
    movie => movie.id === 'watch-chandu-champion-2024'
  )
  return {
    props: {
      movie: selectedMovie
