...
const moviesDetail = ({ movie }) => {
  ...
  const languagesSchema = JSON.stringify({
    ...
    name: 'Watch Movie One Fast Move (2024) | 123Movies™',
    alternateName: [
      'Ver película One Fast Move (2024) | 123Movies™',
      'Regarder le film One Fast Move (2024) | 123Movies™',
      'Film One Fast Move (2024) ansehen | 123Movies™',
      '观看电影 One Fast Move (2024) | 123Movies™',
      '映画 One Fast Move (2024) を見る | 123Movies™',
      '영화 One Fast Move (2024) 보기 | 123Movies™',
      'Assistir Filme One Fast Move (2024) | 123Movies™',
      'Guarda il film One Fast Move (2024) | 123Movies™',
      'Посмотреть фильм One Fast Move (2024) | 123Movies™',
      'مشاهدة فيلم One Fast Move (2024) | جي دبليو إف™'
    ],
    inLanguage: [
      'es',
      'fr',
  ...
  return (
    <div>
      <Head>
        ...
        <title>Watch Movie One Fast Move (2024) | 123Movies™</title>
...
export async function getServerSideProps () {
  ...
  const selectedMovie = data.find(
    movie => movie.id === 'watch-one-fast-move-2024'
  )
  return {
    props: {
      movie: selectedMovie
